...
<br>
<br>
##Many are available for example here :
...
<br>
question 1
It comes from the file dataset1.R, line 8 says mytickers = c("SPY", "AAPL")
...
1.The number of column is : `r ncol(transposedData)` 
and the number of rows is :
`r nrow(transposedData)`

2. Lets apply the "mean" function to the columns of transposeData : 
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio_of_snp_and_apple<-apply(transposedData,2,mean)
names(portfolio_of_snp_and_apple) <- rownames(StockReturns)
pnl_plot(portfolio_of_snp_and_apple)
```
<br>
<br>
<br>
...
**Your Answers here:**
<br>
1.We need to open the file "dataSet1.R" and modify the line 9 from startDate = "1999-01-01" to startDate = "2000-01-01" for example and then re-run the report
<br>
<br>
<br>
2.yes, for that we need to modify the dataSet1.R file again:
line 8 `mytickers = c("SPY", "AAPL")` We can add as many additional company name as we need here and run the report again.
<hr>
<hr>
...
<br>
<br>
when I enter those commands into the consol I get multiple errors...
<hr>
<hr>
