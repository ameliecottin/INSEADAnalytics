...
*1)we need 6 component to explain more than 50% of the data
*2) the highest weight is 0.93 for the first component, then 0,86 and 0,85
*3) The first 10 components are portfolios of futures which behave in a similar way. the factors could be equivalent to the betas of the futures in the portfolios. We would not want to use these components as actual portfolios because they are not diversified. ideally we would try to build a portfolio with futures in each of the components.
*4)
```{r}
...
```{r echo=FALSE}
show_data2 = round(Variance_Explained_Table2, 2)
...
iplot.df(melt(df2, id="components"))
```
*
scree plots crisis
*
...
Variance_Explained_Table3<-Variance_Explained_Table_results3$eig
colnames(Variance_Explained_Table3)<-c("Eigenvalue", 
  "Pct of explained variance", "Cumulative pct of explained variance")
```

```{r echo=FALSE}
show_data3 = round(Variance_Explained_Table3, 2)
iprint.df(show_data3)
```


```{r echo=FALSE}
eigenvalues3  <- Variance_Explained_Table3[, "Eigenvalue"]
```
...
iplot.df(melt(df3, id="components"))
```

*5)
During the bull (pre crisis) years, there are 16 eigenvalues above the abline, and 13 for the bear (crisis) years. It makes sense, as during the crisis, the majority of futures are falling, the assets are more correlated. There is a need for less information to understand what is happening as the volatility is smaller.
*
*
*
*
...
*1) The code constructed a signal matrix based on the summation of signs(votes), and allocate $1 to each of the futures on a daily basis 

*
*2) The order of moving average increases : the higher the order, the more accurate is the result
*
*
*
*
